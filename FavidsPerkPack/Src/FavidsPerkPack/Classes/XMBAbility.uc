...
static function X2AbilityTemplate Passive(name DataName, string IconImage, optional bool bCrossClassEligible = false, optional X2Effect_Persistent Effect = none, optional bool displayEffectAsBonus  = true, optional bool hidePerkWhenNotRelevant = true)
{
									  ...
									  ConditionalEffect.ScaleValue != none))
	{
		...
		ConditionalEffect.SetDisplayInfo(ePerkBuff_Bonus, Template.LocFriendlyName, Template.LocHelpText, Template.IconImage, displayEffectAsBonus,,Template.AbilitySourceName);
		ConditionalEffect.bHideWhenNotRelevant = hidePerkWhenNotRelevant;
...
simulated static function EffectFlyOver_Visualization_Imposition(XComGameState VisualizeGameState, out VisualizationTrack BuildTrack, const name EffectApplyResult)
{
	local X2Action_PlaySoundAndFlyOver	SoundAndFlyOver;
	...
	{
		SoundAndFlyOver = X2Action_PlaySoundAndFlyOver(class'X2Action_PlaySoundAndFlyOver'.static.AddToVisualizationTrack(BuildTrack, VisualizeGameState.GetContext()));
		SoundAndFlyOver.SetSoundAndFlyOverParameters(None, "Imposition", '', eColor_Bad, "img:///UILibrary_FavidsPerkPack.UIPerk_Imposition");
	}
}
