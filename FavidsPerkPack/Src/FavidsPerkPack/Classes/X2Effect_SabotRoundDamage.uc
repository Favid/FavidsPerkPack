class X2Effect_SabotRoundDamage extends X2Effect_Persistent;

function int GetAttackingDamageModifier(XComGameState_Effect EffectState, XComGameState_Unit Attacker, Damageable TargetDamageable, XComGameState_Ability AbilityState, const out EffectAppliedData AppliedData, const int CurrentDamage, optional XComGameState NewGameState)
{
	local XComGameState_Item SourceWeapon;
	local int ExtraDamage;
	local X2ItemTemplate WeaponTemplate;
	
	if (AbilityState.GetMyTemplateName() == 'F_SabotRound')
	{
		SourceWeapon = AbilityState.GetSourceWeapon();

		if (SourceWeapon != none)
		{
			WeaponTemplate = SourceWeapon.GetMyTemplate();
		
			if(WeaponTemplate.Tier == 1) 
			{
				ExtraDamage = class'X2Ability_Favid'.default.SABOTROUND_DAMAGE_BONUS_T1;
			}
			else if(WeaponTemplate.Tier == 2) 
			{
				ExtraDamage = class'X2Ability_Favid'.default.SABOTROUND_DAMAGE_BONUS_T2;
			}
			else if(WeaponTemplate.Tier == 3) 
			{
				ExtraDamage = class'X2Ability_Favid'.default.SABOTROUND_DAMAGE_BONUS_T3;
			}
		}
	}
